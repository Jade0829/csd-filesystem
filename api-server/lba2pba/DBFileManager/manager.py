...
class DBFileManager(lba2pba_pb2_grpc.DBFileManagerServicer):
    def __init__(self):
        ...
        self.FM = {}
        self.pvcInfo = manager_kube.GetPVCInfo()
        self.QAs = {}
    ...
    def RegisterQA(self, request, content):
        ip,port = request.addr.split(":")
        pvcName = manager_kube.GetPVC(ip)

        if not pvcName in self.QAs.keys():
            self.QAs[pvcName] = []

        self.QAs[pvcName].append(f'{ip}:{port}')
        print(f'Register Query Agent : pvcName({pvcName}), addr({ip}:{port})')
    ...
    def DeregisterQA(self, request, content):
        ...
        agents = list(set(self.QAs[pvcName]))
        agents.remove(f'{ip}:{port}')

        self.QAs[pvcName] = agents
        print(f'Deregister Query Agent : pvcName({pvcName}), addr({ip}:{port})')
